...
export async function POST(req: Request) {
  try {
    ...
    // Service role buat read/update products & update order totals
    ...
    // Hitung subtotal dulu (server-side)
    let subtotal = 0;
    for (const it of items) {
      const p = map.get(it.product_id)!;
    ...
    // 1) Create order (langsung simpan totals yg benar)
    ...
    const orderItems = items.map((it) => {
      ...
      return {
        ...
        quantity: it.quantity, // ✅ order_items pakai quantity
      };
    });
    ...
    // 4) (Optional) double-check totals tersimpan (debug)
    ...
    // 5) Midtrans Snap (gross_amount harus = total)
    ...
    const snap = await createMidtransSnap({
      ...
      customer: {
        first_name: String(
          (shipping.name ?? profile?.full_name ?? "Customer")
        )
          .trim()
          .split(" ")[0],
